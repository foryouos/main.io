...
卡片图标为**官方微信公众号 logo**（Simple Icons 的 WeChat 路径，内联 SVG，24 视口），配色对齐你提供的「扫码_搜索联合传播样式」参考图——绿色气泡在前、浅灰气泡在后：
...
> 品牌绿 `#07C160` 是四张卡片里唯一的非橙/金色。这是有意的：本卡片是公众号入口，且与浮层里的绿色按钮同色。若要回到单色体系，删掉 `.card-wx .orb .wx-mark-*` 三条 `fill` 规则，改为 `fill: currentColor` 即可。
